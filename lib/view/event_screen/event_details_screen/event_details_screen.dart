...
class _EventDetailsScreenState extends State<EventDetailsScreen> {
  ...
  Future<void> _initDatabase() async {
    _database = await openDatabase(
      ...
      onCreate: (db, version) {
        return db.execute(
          'CREATE TABLE messages(id INTEGER PRIMARY KEY AUTOINCREMENT, communityName TEXT, message TEXT)',
        );
      },
      version: 1,
    );
  ...
  Future<void> _loadMessages() async {
    final List<Map<String, dynamic>> maps = await _database.query(
      'messages',
      where: 'communityName = ?',
      whereArgs: [widget.communityName],
    );
  ...
  Future<void> _addMessage(String message) async {
    await _database.insert(
      'messages',
      {'communityName': widget.communityName, 'message': message},
